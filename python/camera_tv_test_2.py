...
With default 90° rotation, horizontal mirror before rotation,
and ‘r’ key to toggle back to horizontal view.
"""
...
class FaceShoulderApp:
	def __init__(self):
		...
		# Default to portrait (rotate output 90°)
		...
		print(f"Display: {self.display_width}x{self.display_height}, rotate90={self.rotate_90_degrees}, mirror={self.mirror_horizontally}")
	...
	def setup_imx500_camera(self):
		try:
			...
			self.intrinsics.inference_rate = self.intrinsics.inference_rate or 30
			...
			print("IMX500 ready:", self.actual_camera_resolution, self.intrinsics.inference_rate)
		...
		try:
			...
			if outputs is not None:
				...
				if scores is not None and len(scores)>0:
					...
					self.pose_detected=False; self.current_frame_crop_region=None
			else:
				self.pose_detected=False; self.current_frame_crop_region=None
		except Exception as e:
	...
	def _calculate_face_shoulder_crop_region(self):
		if not self.pose_detected: return None
		...
		if crop_aspect>disp_aspect:
			new_h = cw/disp_aspect; dy=(new_h-ch)/2
			cy=max(0,cy-dy); ch=min(self.camera_resolution[1]-cy,new_h)
		else:
			new_w = ch*disp_aspect; dx=(new_w-cw)/2
			cx=max(0,cx-dx); cw=min(self.camera_resolution[0]-cx,new_w)
	...
	def scale_with_aspect_ratio(self,surf,tw,th):
		sw,sh=surf.get_size();
		if sw/sh>tw/th: nw,nh=tw,int(tw*sh/sw)
		else: nh,nw=th,int(th*sw/sh)
		s=pygame.transform.scale(surf,(nw,nh))
		f=pygame.Surface((tw,th)); f.fill(self.black)
		f.blit(s,((tw-nw)//2,(th-nh)//2)); return f

	def draw_video_display(self):
		self.update_fps(); self.screen.fill(self.black)
		...
		try:
			...
			if crop and self.pose_detected:
				x,y,w,h=crop
				if w>0 and h>0:
					cs=pygame.Surface((w,h)); cs.blit(surf,(0,0),(x,y,w,h))
					ds=pygame.transform.scale(cs,(self.display_width,self.display_height))
				else: ds=self.scale_with_aspect_ratio(surf,self.display_width,self.display_height)
			else:
				ds=self.scale_with_aspect_ratio(surf,self.display_width,self.display_height)
			if self.mirror_horizontally:
				ds=pygame.transform.flip(ds,True,False)
			if self.rotate_90_degrees:
				ds=pygame.transform.rotate(ds,90)
			self.screen.blit(ds,(0,0))
			if self.debug_mode: self.draw_pose_debug_overlay()
	...
	def draw_pose_debug_overlay(self):
		if not self.pose_detected: return
		idx=np.argmax(self.last_scores); pts=self.last_keypoints[idx]
		inds=[0,1,2,3,4,5,6]; crop=self.get_face_shoulder_crop_region()
		scr_pts={}
		for j in inds:
			x,y,v=pts[j]
			if v>self.visibility_threshold:
				if crop:
					cx,cy,cw,ch=crop; rx,ry=(x-cx)/cw,(y-cy)/ch
		...
		for a,b in [(0,1),(0,2),(1,3),(2,4)]:
			if a in scr_pts and b in scr_pts:
				pygame.draw.line(self.screen,self.white,scr_pts[a],scr_pts[b],2)
		if 5 in scr_pts and 6 in scr_pts:
			pygame.draw.line(self.screen,self.green,scr_pts[5],scr_pts[6],2)
		for j in inds:
			if j in scr_pts:
	...
	def draw_status_overlay(self):
		font=pygame.font.Font(None,48)
		fps=f"FPS: {self.current_fps:.1f}"
		if self.rotate_90_degrees:
			txt=pygame.transform.rotate(font.render(fps,True,self.white),90)
			pos=(self.display_width-20,20)
		else:
			txt=font.render(fps,True,self.white);
		...
		if hasattr(self,'actual_camera_resolution'):
			...
			if self.rotate_90_degrees:
				...
				rt=font.render(res,True,self.white); rp=(self.display_width-300,70)
	...
	def handle_events(self):
		for e in pygame.event.get():
			...
			if e.type==pygame.KEYDOWN:
				...
				if e.key==pygame.K_d:
					self.debug_mode=not self.debug_mode; print("Debug mode",self.debug_mode)
				if e.key==pygame.K_r:
					# toggle rotation
					self.rotate_90_degrees=not self.rotate_90_degrees
					if self.rotate_90_degrees:
						self.display_width,self.display_height=self.fullhd_height,self.fullhd_width
					else:
						self.display_width,self.display_height=self.fullhd_width,self.fullhd_height
...
if __name__=="__main__":
	try:
		app=FaceShoulderApp(); app.run()
	except KeyboardInterrupt:
		print("Interrupted, exiting...")
	except Exception as e:
		print(f"Error: {e}", file=sys.stderr)
		print("Ensure Raspberry Pi + IMX500 + model installed")